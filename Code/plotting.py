...
states = np.genfromtxt(os.path.join(file_path,"states.txt").replace("\\.", "."), delimiter='\t',skip_header=1)
...
dep_vars = np.genfromtxt(os.path.join(file_path,"dep_vars.txt").replace("\\.", "."), delimiter='\t',skip_header=1)
print(dep_vars)
time = dep_vars[:,0]
